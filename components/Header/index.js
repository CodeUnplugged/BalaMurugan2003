...
const Header = ({ data: { menus, handleItemSelect, handleIconClick, rightBtn, logo } }) => {
    ...
    return (
      ...
      <button
  onClick={() => {
    const form = document.getElementById("ai-contact-form"); // Add the correct form ID
    if (form) {
      const offset = 110; // Adjust the offset value as needed
      const formPosition = form.getBoundingClientRect().top + window.scrollY - offset;
      window.scrollTo({ top: formPosition, behavior: "smooth" });

      // Add the animation class for highlighting
      form.classList.add("form-highlight");
      setTimeout(() => form.classList.remove("form-highlight"), 1000); // Remove the class after 1s
    }
  }}
